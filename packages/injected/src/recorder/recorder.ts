...
class InspectTool implements RecorderTool {
  ...
  onMouseMove(event: MouseEvent) {
    ...
    if (this._hoveredElement) {
      ...
      model = {
        ...
        color: this._assertVisibility
          ? getHighlightColors().assert
          : getHighlightColors().single,
      };
    }

    if (this._hoveredModel?.selector === model?.selector) return;
  ...
  onMouseLeave(event: MouseEvent) {
    ...
    if (
      window.top !== window &&
      this._recorder.deepEventTarget(event).nodeType === Node.DOCUMENT_NODE
    )
      this._reset(true);
  }
  ...
  onKeyDown(event: KeyboardEvent) {
    consumeEvent(event);
    if (event.key === "Escape") {
      if (this._assertVisibility) this._recorder.setMode("recording");
    }
  }
...
class RecordActionTool implements RecorderTool {
  ...
  private _pendingClickAction:
    | { action: actions.ClickAction; timeout: number }
    | undefined;
  ...
  private _installObserverIfNeeded() {
    ...
    if (!this._recorder.injectedScript.document?.body) return;
    this._observer = new MutationObserver((mutations) => {
      if (!this._hoveredElement) return;
      for (const mutation of mutations) {
        for (const node of mutation.removedNodes) {
          if (
            node === this._hoveredElement ||
            node.contains(this._hoveredElement)
          )
  ...
  onClick(event: MouseEvent) {
    ...
    if (isRangeInput(this._hoveredElement)) return;
  ...
  private _cancelPendingClickAction() {
    if (this._pendingClickAction)
      this._recorder.injectedScript.utils.builtins.clearTimeout(
        this._pendingClickAction.timeout,
      );
  ...
  private _showActionListDialog(
    model: HighlightModelWithSelector,
    event: MouseEvent,
  ) {
    consumeEvent(event);
    ...
    for (const action of actions) {
      const actionElement = factories.createActionItem(
    ...
    const anchorBox =
      this._recorder.highlight.firstTooltipBox() ||
      model.elements[0].getBoundingClientRect();
    const dialogPosition = this._recorder.highlight.tooltipPosition(
      anchorBox,
      dialogElement,
    );
  ...
  private _onFocus(userGesture: boolean) {
    ...
    if (userGesture && activeElement === this._recorder.document.body) return;
    const result = activeElement
      ? this._recorder.injectedScript.generateSelector(activeElement, {
          testIdAttributeName: this._recorder.state.testIdAttributeName,
        })
      : null;
    this._activeModel =
      result && result.selector
        ? { ...result, color: getHighlightColors().action }
        : null;
    if (userGesture) {
  ...
  private _shouldIgnoreMouseEvent(event: MouseEvent): boolean {
    ...
    if (nodeName === "SELECT" || nodeName === "OPTION") return true;
    if (
      nodeName === "INPUT" &&
      ["date", "range"].includes((target as HTMLInputElement).type)
    )
      return true;
    return false;
  }
  ...
  private _consumedDueToNoModel(
    event: Event,
    model: HighlightModel | null,
  ): boolean {
  ...
  private _consumedDueWrongTarget(event: Event): boolean {
    if (
      this._activeModel &&
      this._activeModel.elements[0] === this._recorder.deepEventTarget(event)
    )
      return false;
    consumeEvent(event);
    return true;
  ...
  private _consumeWhenAboutToPerform(event: Event) {
    if (!this._performingActions.size) consumeEvent(event);
  }
  ...
  private _performAction(action: actions.PerformOnRecordAction) {
    ...
    if (!this._recorder.injectedScript.isUnderTest) return;
        ...
        "Action performed for test: " +
          JSON.stringify({
            ...
            hovered: this._hoveredModel
              ? (this._hoveredModel as any).selector
              : null,
            active: this._activeModel
              ? (this._activeModel as any).selector
              : null,
          }),
      );
    });
  ...
  private _shouldGenerateKeyPressFor(event: KeyboardEvent): boolean {
    ...
    if (
      event.key === "Enter" &&
      (this._recorder.deepEventTarget(event).nodeName === "TEXTAREA" ||
        this._recorder.deepEventTarget(event).isContentEditable)
    )
      return false;
  ...
  private _updateModelForHoveredElement() {
    ...
    if (this._hoveredModel && this._hoveredModel.selector === selector) return;
    this._hoveredModel = selector
      ? { selector, elements, color: getHighlightColors().action }
      : null;
    this._updateHighlight(true);
  }
...
class JsonRecordActionTool implements RecorderTool {
  ...
  onClick(event: MouseEvent) {
    ...
    if (isRangeInput(element)) return;
    // Right clicks are handled by 'contextmenu' event if its auxclick
    if (event.button === 2 && event.type === "auxclick") return;
    if (this._shouldIgnoreMouseEvent(event)) return;
  ...
  onInput(event: Event) {
    ...
    if (
      ...
      this._recorder.recordAction({
        ...
        ariaSnapshot,
        signals: [],
        text: element.isContentEditable
          ? element.innerText
  ...
  private _ariaSnapshot(element: HTMLElement): {
    ariaSnapshot: string;
    selector: string;
    ref?: string;
  };
  private _ariaSnapshot(element: HTMLElement | undefined): {
    ariaSnapshot: string;
    selector?: string;
    ref?: string;
  } {
    const { ariaSnapshot, refs } =
      this._recorder.injectedScript.ariaSnapshotForRecorder();
    const ref = element ? refs.get(element) : undefined;
    const elementInfo = element
      ? this._recorder.injectedScript.generateSelector(element, {
          testIdAttributeName: this._recorder.state.testIdAttributeName,
        })
      : undefined;
...
class TextAssertionTool implements RecorderTool {
  ...
  onClick(event: MouseEvent) {
    ...
    if (this._kind === "value") {
      ...
      if (!this._dialog.isShowing()) this._showDialog();
    }
  }

  onMouseDown(event: MouseEvent) {
    const target = this._recorder.deepEventTarget(event);
    if (this._elementHasValue(target)) event.preventDefault();
  }
  ...
  onMouseMove(event: MouseEvent) {
    if (this._dialog.isShowing()) return;
    const target = this._recorder.deepEventTarget(event);
    if (this._hoverHighlight?.elements[0] === target) return;
    if (this._kind === "text" || this._kind === "snapshot") {
      this._hoverHighlight = this._recorder.injectedScript.utils.elementText(
        ...
        ? {
            elements: [target],
            selector: "",
            color: getHighlightColors().assert,
          }
        : null;
  ...
  onKeyDown(event: KeyboardEvent) {
    if (event.key === "Escape") this._recorder.setMode("recording");
    consumeEvent(event);
  }
  ...
  private _generateAction(): actions.AssertAction | null {
    ...
    if (this._kind === "value") {
      if (!this._elementHasValue(target)) return null;
      const { selector } = this._recorder.injectedScript.generateSelector(
        target,
        { testIdAttributeName: this._recorder.state.testIdAttributeName },
      );
      if (
      ...
      return {
        ...
        signals: [],
        ariaSnapshot: this._recorder.injectedScript.ariaSnapshot(target, {
          mode: "codegen",
        }),
      };
    } else {
  ...
  private _renderValue(action: actions.Action) {
    if (action?.name === "assertText")
      return this._recorder.injectedScript.utils.normalizeWhiteSpace(
        action.text,
      );
    if (action?.name === "assertChecked") return String(action.checked);
    if (action?.name === "assertValue") return action.value;
    if (action?.name === "assertSnapshot") return action.ariaSnapshot;
    return "";
  }

  private _commit() {
    if (!this._action || !this._dialog.isShowing()) return;
    this._dialog.close();
  ...
  private _showDialog() {
    if (!this._hoverHighlight?.elements[0]) return;
  ...
  private _showTextDialog(action: actions.AssertTextAction) {
    ...
    const updateAndValidate = () => {
      const newValue = this._recorder.injectedScript.utils.normalizeWhiteSpace(
        textElement.value,
      );
      const target = this._hoverHighlight?.elements[0];
      if (!target) return;
      action.text = newValue;
      const targetText = this._recorder.injectedScript.utils.elementText(
        this._textCache,
        target,
      ).normalized;
    ...
    const position = this._recorder.highlight.tooltipPosition(
...
class Overlay {
  ...
  constructor(recorder: Recorder) {
    ...
    this._pickLocatorToggle = factories.createToolItem(
      document,
      "pick-locator",
      "Pick locator",
    );
    ...
    this._assertVisibilityToggle = factories.createToolItem(
      document,
      "visibility",
      "Assert visibility",
    );
    ...
    this._assertTextToggle = factories.createToolItem(
      document,
      "text",
      "Assert text",
    );
    ...
    this._assertValuesToggle = factories.createToolItem(
      document,
      "value",
      "Assert value",
    );
    ...
    this._assertSnapshotToggle = factories.createToolItem(
      document,
      "snapshot",
      "Assert snapshot",
    );
    ...
    this._listeners = [
      addEventListener(this._dragHandle, "mousedown", (event) => {
        this._dragState = {
          ...
          dragStart: { x: (event as MouseEvent).clientX, y: 0 },
        };
      }),
      addEventListener(this._recordToggle, "click", () => {
        if (this._recordToggle.classList.contains("disabled")) return;
  ...
  setUIState(state: UIState) {
    const isRecording =
      ...
      state.mode === "assertingText" ||
      state.mode === "assertingVisibility" ||
      state.mode === "assertingValue" ||
      state.mode === "assertingSnapshot" ||
      state.mode === "recording-inspecting";
    this._recordToggle.classList.toggle("toggled", isRecording);
    this._recordToggle.title = isRecording
      ? "Stop Recording"
      : "Start Recording";
    ...
    if (state.mode === "none") this._hideOverlay();
  ...
  private _updateVisualPosition() {
    ...
    this._overlayElement.style.left =
      (this._recorder.injectedScript.window.innerWidth - this._measure.width) /
        2 +
      this._offsetX +
      "px";
  }
  ...
  onMouseMove(event: MouseEvent) {
    ...
    if (this._dragState) {
      this._offsetX =
        this._dragState.offsetX + event.clientX - this._dragState.dragStart.x;
      const halfGapSize =
        (this._recorder.injectedScript.window.innerWidth -
          this._measure.width) /
          2 -
        10;
      this._offsetX = Math.max(
        -halfGapSize,
        Math.min(halfGapSize, this._offsetX),
      );
      this._updateVisualPosition();
...
export class Recorder {
  ...
  installListeners() {
    ...
    this._listeners = [
      ...
      addEventListener(
        this.document,
        "dblclick",
        (event) => this._onDblClick(event as MouseEvent),
        true,
      ),
      addEventListener(
        this.document,
        "contextmenu",
        (event) => this._onContextMenu(event as MouseEvent),
        true,
      ),
      addEventListener(
        this.document,
        "dragstart",
        (event) => this._onDragStart(event as DragEvent),
        true,
      ),
      addEventListener(
        this.document,
      ...
      addEventListener(
        this.document,
        "pointerdown",
        (event) => this._onPointerDown(event as PointerEvent),
        true,
      ),
      addEventListener(
        this.document,
        "pointerup",
        (event) => this._onPointerUp(event as PointerEvent),
        true,
      ),
      addEventListener(
        this.document,
      ...
      addEventListener(
        this.document,
        "mousemove",
        (event) => this._onMouseMove(event as MouseEvent),
        true,
      ),
      addEventListener(
        this.document,
      ...
      addEventListener(
        this.document,
        "mouseenter",
        (event) => this._onMouseEnter(event as MouseEvent),
        true,
      ),
      addEventListener(
        this.document,
        "focus",
        (event) => this._onFocus(event),
        true,
      ),
      addEventListener(
        this.document,
        "scroll",
        (event) => this._onScroll(event),
        true,
      ),
    ];
    ...
    const recreate = () => {
      this.highlight.install();
      recreationInterval = this.injectedScript.utils.builtins.setTimeout(
        recreate,
        500,
      );
    };
    ...
    this._listeners.push(() =>
      this.injectedScript.utils.builtins.clearTimeout(recreationInterval),
    );
  ...
  private _switchCurrentTool() {
    ...
    if (newTool === this._currentTool) return;
  ...
  setUIState(state: UIState, delegate: RecorderDelegate) {
    ...
    if (
      ...
      state.actionPoint.x === this.state.actionPoint.x &&
      state.actionPoint.y === this.state.actionPoint.y
    ) {
      // All good.
      ...
      if (state.actionPoint)
        this.highlight.showActionPoint(
          state.actionPoint.x,
          state.actionPoint.y,
        );
      else this.highlight.hideActionPoint();
    }
    ...
    if (state.actionSelector !== this._lastHighlightedSelector) {
      const entries = state.actionSelector
        ? entriesForSelectorHighlight(
            this.injectedScript,
            state.language,
      ...
      this._lastHighlightedSelector = entries?.length
        ? state.actionSelector
        : undefined;
    }
    ...
    if (this._lastHighlightedAriaTemplateJSON !== ariaTemplateJSON) {
      const elements = state.ariaTemplate
      ...
      if (elements.length) {
        const color =
          elements.length > 1
            ? getHighlightColors().multiple
            : getHighlightColors().single;
        highlight = elements.map((element) => ({ element, color }));
        this._lastHighlightedAriaTemplateJSON = ariaTemplateJSON;
      } else {
        if (!this._lastHighlightedSelector) highlight = "clear";
        this._lastHighlightedAriaTemplateJSON = "undefined";
      }
    }

    if (highlight === "clear") this.highlight.clearHighlight();
    else if (highlight !== "noop") this.highlight.updateHighlight(highlight);
  }
  ...
  private _onClick(event: MouseEvent) {
    ...
    if (this.overlay?.onClick(event)) return;
    if (this._ignoreOverlayEvent(event)) return;
    this._currentTool.onClick?.(event);
  }

  private _onDblClick(event: MouseEvent) {
    if (!event.isTrusted) return;
    if (this.overlay?.onDblClick(event)) return;
    if (this._ignoreOverlayEvent(event)) return;
  ...
  private _onMouseUp(event: MouseEvent) {
    if (!event.isTrusted) return;
    if (this.overlay?.onMouseUp(event)) return;
    if (this._ignoreOverlayEvent(event)) return;
  ...
  private _onMouseMove(event: MouseEvent) {
    if (!event.isTrusted) return;
    if (this.overlay?.onMouseMove(event)) return;
    if (this._ignoreOverlayEvent(event)) return;
  ...
  private _updateHighlight(model: HighlightModel | null, userGesture: boolean) {
    ...
    if (tooltipText === undefined && model?.selector)
      tooltipText = this.injectedScript.utils.asLocator(
        this.state.language,
        model.selector,
      );
    if (model)
    ...
    else this.highlight.clearHighlight();
  ...
  private _captureAutoExpectSnapshot() {
    ...
    return documentElement
      ? this.injectedScript.utils.generateAriaTree(documentElement, {
          mode: "autoexpect",
        })
      : undefined;
  }
  ...
  async performAction(action: actions.PerformOnRecordAction) {
    ...
    if (!isAssertAction(action) && this._lastActionAutoexpectSnapshot) {
      const element = this.injectedScript.utils.findNewElement(
        previousSnapshot?.root,
        this._lastActionAutoexpectSnapshot?.root,
      );
      action.preconditionSelector = element
        ? this.injectedScript.generateSelector(element, {
...
class Dialog {
  ...
  show(options: {
    ...
    this._dialogElement = factories.createDialog(
      this._recorder.document,
      !!options.autosize,
    );
    ...
    this._keyboardListener = (event: KeyboardEvent) => {
      ...
      if (
        ...
        if (this._dialogElement) options.onCommit();
        return;
      }
    };
    ...
    this._dialogElement.addEventListener("click", (event) =>
...
function asCheckbox(node: Node | null): HTMLInputElement | null {
  ...
  return ["checkbox", "radio"].includes(inputElement.type)
    ? inputElement
    : null;
}
...
function entriesForSelectorHighlight(
  injectedScript: InjectedScript,
  language: Language,
  selector: string,
  ...
  try {
    ...
    const elements = injectedScript.querySelectorAll(
      parsedSelector,
      ownerDocument,
    );
    const color =
    ...
    return elements.map((element, index) => {
      const suffix =
        elements.length > 1 ? ` [${index + 1} of ${elements.length}]` : "";
...
function createSvgElement(
  doc: Document,
  { tagName, attrs, children }: SvgJson,
): SVGElement {
  ...
  if (children) {
    for (const c of children) elem.appendChild(createSvgElement(doc, c));
  }

  return elem;
}

function isAssertAction(
  action: actions.Action,
): action is actions.AssertAction {
